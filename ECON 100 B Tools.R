#
#  ECON 100 B Tools.R        <<-- SAVE THE FILE WITH THIS NAME IN YOUR RStuio.Cloud 
#  
#  Spring 2023 | Bard College
#
...
# 12. cpi.plot(s)      -- chart of three CPI items       
...
cpi.plot <- function(s) {
  ...
  cpi <- bls_request(query_n_series(s), api_key = key.BLS)
  # Turning list of lists (cpi) into data.frame (df)
  data_x <- lapply(1:length(cpi[["series"]]), \(x) 
                   as.data.frame(do.call(rbind, cpi[["series"]][[x]][["data"]]))[-5])
  data_xx <- lapply(1:length(data_x), \(x) cbind(seriesID = cpi[["series"]][[x]][["seriesID"]], data_x[[x]]))
  cpi <- as.data.frame(do.call(rbind, data_xx))
  
  # Adding new column item_name, containing ID names, into the cpi data set
  cpi <- bls.cpi.item_name(cpi, s)
  cpi$date <- as.Date(paste0(cpi$year,"-",substr(cpi$period,2,3),"-01"))
  cpi <- cpi[order(cpi$date),]
  
  cpi.ts <- ts(data.frame(cpi1=as.numeric(cpi[cpi$seriesID==s[1],]$value), 
                          cpi2=as.numeric(cpi[cpi$seriesID==s[2],]$value),
                          cpi3=as.numeric(cpi[cpi$seriesID==s[3],]$value)), 
               start=c(year(cpi[cpi$seriesID==s[1],]$date[1]),
                       month(cpi[cpi$seriesID==s[1],]$date[1])),
               frequency = 12)
  cpi.yoy.ts <- (cpi.ts/stats::lag(cpi.ts,-12)-1)*100
  
  library(zoo); library(lubridate)
  d <- lubridate::date_decimal(as.numeric(time(cpi.yoy.ts)))
  d <- as.Date(substr(as.character(d),1,10))
  
  # --------------- Standard dimensions for plot_ly() charts ---------------------+
  w = 1000; h = 450 # width and height of the charts by 
  m <- list( # Margins for the charts created by plot_ly()
    l = 50,
    r = 50,
    b = 100,
    t = 100,
  ...
  print(cpi[cpi$seriesID==s[1],]$item_name[1])
  print(round(cpi.yoy.ts[,1],2))
  print(cpi[cpi$seriesID==s[2],]$item_name[1])
  print(round(cpi.yoy.ts[,2],2))
  print(cpi[cpi$seriesID==s[3],]$item_name[1])
  print(round(cpi.yoy.ts[,3],2))
  
  g <- 
    plotly::plot_ly(x = ~d, 
                  y = ~as.numeric(cpi.yoy.ts[,1]), 
                  color = "blue", 
                  name = cpi[cpi$seriesID==s[1],]$item_name[1],
                  type = "scatter", mode = "lines+markers", 
                  width = w, height = h) %>% 
    add_trace(y = ~as.numeric(cpi.yoy.ts[,2]), 
              color = "black", name = cpi[cpi$seriesID==s[2],]$item_name[1], 
              mode = 'lines+markers') %>%
    add_trace(y = ~as.numeric(cpi.yoy.ts[,3]), 
              color = "red", name = cpi[cpi$seriesID==s[3],]$item_name[1], 
              mode = 'lines+markers') %>%
    layout(title = "All Items CPI & Selected Components",
           xaxis = list(title = ""), 
